...
func _physics_process(delta):
    ...
    $Aoe/CollisionShape3D.position.y = max_distance / 2 + 1a.0 # 2 is margin
    for body in aoe.get_overlapping_bodies():
        if body is Summoner:
            ...
            var attenuation: float = 1.0-clamp((body.position.distance_to(position) / max_distance), 0.0, 1.0)
